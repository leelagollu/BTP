...
function get_data(){
  console.log("entered");
  $.getScript('static/js/new.js', function() {
...
function runmainfunction(){
  ...
  obtaincoordinates = (reference,vertical,horizontal) => {
       const R=6378137;  //Earth’s radius, sphere
       //Coordinate offsets in radians
       var dLat = vertical/R ;                     
       var dLon = horizontal/(R*Math.cos(Math.PI*(reference.lat)/180)) ;   
       //OffsetPosition, decimal degrees
       latA = reference.lat + dLat * 180/Math.PI ;
       lonA = reference.lon + dLon * 180/Math.PI;
...
function getpolygondata(){
  ...
  for(let i=0;i<=floors.length;i++){
    polygon_url="http://inclunav.apps.iitd.ac.in/node/wayfinding/v1/all-polygons/"+venue_name+"/"+building_name+"/"+fdata[i];
